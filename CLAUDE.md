...
编写 SQL，输出完整 SQL 脚本供用户确认
    ↓
用户确认 SQL 后，判断是否需要归档：
    - 若 SQL 是**全新逻辑**（无历史脚本可复用）→ 自动保存到 ./queries/ 并更新 SQL_INDEX.md
    - 若 SQL **复用已有脚本**（仅修改查询条件，如门店号、日期范围等）→ 询问用户是否保存副本，默认不保存
    ↓
...
保存到 ./output/ 目录，文件名带日期（格式：描述_YYYYMMDD.csv）
    - 小数据量：MCP 返回内联 JSON → 直接生成 CSV
    - 大数据量（数千行+）：MCP 自动保存临时文件 → 调用 `node scripts/json-to-csv.js <临时文件> <输出CSV>` 转换
    ↓
告知用户文件路径、行数、大小等摘要信息
```
